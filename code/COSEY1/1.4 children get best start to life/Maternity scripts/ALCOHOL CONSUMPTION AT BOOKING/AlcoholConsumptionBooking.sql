...
with cte as (
select * from 
(select  ROW_NUMBER() OVER (PARTITION BY [Hospital number], [Pregnancy number], [Estimate of due date (agreed)],
Postcode ORDER BY [Hospital number] ) as Sqnce,[Date of booking], [Hospital number], [Pregnancy number],
[Ethnic category], [Alcohol consumption (u/week)] , coalesce( [Estimate of due date (agreed)], [Estimate of due date from LMP]) as EDD, 
[Gravida],Postcode
...
,cte2 as(
select * from  
(select ROW_NUMBER() OVER (PARTITION BY [Hospital number],[Pregnancy number], [Date of booking] 
ORDER BY [Hospital number] ) as Sqnce2, *
from cte 
) bb
where Sqnce2 =1
)
...
,cte5 as(select Date_Time_Of_Booking, HospitalNumberMother, Pregnancy_ID, Ethnic_Category,[Alcohol_Consumption_Units],
